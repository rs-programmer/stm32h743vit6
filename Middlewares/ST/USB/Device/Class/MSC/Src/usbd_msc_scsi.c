...
static int8_t SCSI_CheckAddressRange(
    USBD_HandleTypeDef *pdev, uint8_t lun, uint32_t blk_offset, uint32_t blk_nbr);
...
static int8_t SCSI_Inquiry(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *params) {
    ...
    if ((params[1] & 0x01U) != 0U) /* Evpd is set */
    {
        ...
        {
            SCSI_SenseCode(pdev, hmsc->cbw.bLUN, ILLEGAL_REQUEST, INVALID_FIELD_IN_COMMAND);
...
static int8_t SCSI_ReadCapacity10(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *params) {
    ...
    ret = ((USBD_StorageTypeDef *)pdev->pUserData[pdev->classId])
              ->GetCapacity(lun, &p_scsi_blk->nbr, &p_scsi_blk->size);
...
static int8_t SCSI_Read10(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *params) {
    ...
    if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
    {
        ...
        p_scsi_blk->addr = ((uint32_t)params[2] << 24) | ((uint32_t)params[3] << 16) |
...
static int8_t SCSI_Read12(USBD_HandleTypeDef *pdev, uint8_t lun, uint8_t *params) {
    ...
    if (hmsc->bot_state == USBD_BOT_IDLE) /* Idle */
    {
        ...
        p_scsi_blk->len = ((uint32_t)params[6] << 24) | ((uint32_t)params[7] << 16) |
 ...
 * @retval status
 */
static int8_t SCSI_ReceiveDiagnosticResults(
...
static int8_t SCSI_ProcessRead(USBD_HandleTypeDef *pdev, uint8_t lun) {
    ...
    if (((USBD_StorageTypeDef *)pdev->pUserData[pdev->classId])
            ->Read(lun, hmsc->bot_data, p_scsi_blk->addr, (len / p_scsi_blk->size)) < 0) {
...
static int8_t SCSI_ProcessWrite(USBD_HandleTypeDef *pdev, uint8_t lun) {
    ...
    if (((USBD_StorageTypeDef *)pdev->pUserData[pdev->classId])
            ->Write(lun, hmsc->bot_data, p_scsi_blk->addr, (len / p_scsi_blk->size)) < 0) {
 ...
 * @retval status
 */
static int8_t SCSI_UpdateBotData(
    USBD_MSC_BOT_HandleTypeDef *hmsc, uint8_t *pBuff, uint16_t length) {
    uint16_t len = length;
