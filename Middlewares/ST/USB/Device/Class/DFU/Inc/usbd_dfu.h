...
#define USBD_DFU_APP_DEFAULT_ADD                                                                   \
    0x08008000U /* The first sector (32 KB) is reserved for DFU code                               \
                 */
...
typedef struct {
    ...
    uint16_t (*VendorDownloadCMD)(
        uint8_t *pbuf, uint32_t BlockNumber, uint32_t wlength, uint32_t *status);
