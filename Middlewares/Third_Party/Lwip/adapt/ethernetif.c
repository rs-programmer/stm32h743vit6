 ...
 * @retval None
 */
void HAL_ETH_RxCpltCallback(ETH_HandleTypeDef *handlerEth) { osSemaphoreRelease(RxPktSemaphore); }
/**
 * @brief  Ethernet Tx Transfer completed callback
 * @param  handlerEth: ETH handler
 * @retval None
 */
void HAL_ETH_TxCpltCallback(ETH_HandleTypeDef *handlerEth) { osSemaphoreRelease(TxPktSemaphore); }
/**
...
static err_t low_level_output(struct netif *netif, struct pbuf *p) {
    ...
    for (q = p; q != NULL; q = q->next) {
        ...
        SCB_CleanDCache_by_Addr((uint32_t *)Txbuffer[i].buffer, Txbuffer[i].len);
...
void HAL_ETH_RxLinkCallback(void **pStart, void **pEnd, uint8_t *buff, uint16_t Length) {
    ...
    *ppEnd = p;
    ...
    /* Update the total length of all the buffers of the chain. Each pbuf in the chain should have
     * its tot_len set to its own length, plus the length of all the following pbufs in the chain.
     */
